...
//   アニメーション番号, UVマップ横サイズ, UVマップ縦サイズ, 横に並ぶアニメーション数
//===========================================================================================
void Sprite::DrawSpriteAnimationRotationReverse(float PosX, float PosY, float Size_W, float Size_H,
	float Kakudo, const D3DXCOLOR& col,
	float PaternNo, float uv_w, float uv_h, int NumPatern)
{
	D3D11_MAPPED_SUBRESOURCE msr;
	...
	//UV座標の書き換え
	vertex[0].TexCoord = D3DXVECTOR2(u + uv_w, v);				//LEFT-TOP
	vertex[1].TexCoord = D3DXVECTOR2(u, v);		//RIGHT-TOP
	vertex[2].TexCoord = D3DXVECTOR2(u + uv_w, v + uv_h);		//LEFT-BOTTOM
	vertex[3].TexCoord = D3DXVECTOR2(u, v + uv_h);	//RIGHT-BOTTOM
