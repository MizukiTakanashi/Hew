...
// 作成者：高梨水希
//============================================================
#include "mercury_collision_all.h"
#include "collision.h"
...
// デフォルトコンストラクタ
//==========================
MercuryCollisionAll::MercuryCollisionAll()
{
...
// 引数付きコンストラクタ
//==========================
MercuryCollisionAll::MercuryCollisionAll(Player* pPlayer, inhPlayerArmBoth* pL, inhPlayerArmBoth* pR,
...
//================================================
int MercuryCollisionAll::Collision(void)
{
	//プレイヤー自身が受けたダメージ数
	int attacked = 0;
	...
	for (int k = 0; k < m_enemy_num; k++) {
		...
		for (int j = 0; j < m_pEnemy[k]->GetOtherNum(); j++) {
			...
			case EnemyManagement::TYPE::GRENADE:
				if (Collision::ColBox(m_pPlayer->GetPos(), m_pEnemy[k]->GetOtherPos(j),
					m_pPlayer->GetSize() / 3, /*EnemyGrenadeManagement::OTHER_RANGE*/m_pEnemy[k]->GetOtherPos(j))) {
...
// プレイヤーのHPが回復する当たり判定
//=======================================
void MercuryCollisionAll::HeelCollision(void)
{
